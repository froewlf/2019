...
* **RDB**是redis默认的持久化方式，安装一定的时间周期策略把内存的数据以快照的形式保存到硬盘二进制文件。即Snapshot快照存储，对应产生的数据文件为dump.rbd，通过胚子文件中的save参数来定义快照的周期。（快照可以是其所表示的数据的一个副本，也可以是数据的一个复制品）
  * RDB的优点
    * RDB非常适用灾难恢复，可随时将数据集还原到不同的版本。
  * RDB的缺点
    * RDB文件需要保存整个数据集的状态，因此可能会至少5分钟保存一次RDB文件。**一旦发生故障停机，你就可能会丢失好几分钟的数据**。
* RDB恢复时对过期键的处理
  * 主恢复时，会过滤掉过期的key
  * 从恢复时，全部恢复
* **AOF**：Redis会将每一个收到的写命令都通过writer函数追加到文件最后，类似于MySQL的binlog。当Redis重启时会通过重新执行文件中保存的写命令来在内存中重建整个数据库的内容。
  * AOF优点
    * AOF可以设置不同的fsync策略，AOF的默认策略为每秒钟fsync一次，在这种配置下，Redis仍然可以保证良好的性能，并且就算发生故障停机，也**最多只会丢失一秒钟的数据**（fsync会在后台线程执行，所以主线程可以继续努力的处理命令请求）
  * AOF缺点
    * 文件体积大
    * 根据所使用的fsync策略，AOF的速度可能会慢于RDB。在一般情况下，每秒fsync的性能依然非常高。
  * AOF恢复数据时对过期键的处理
    * 过期键没有被定期删除或惰性删除，AOF不会对过期键做任何处理
    * 过期键被定期删除或惰性删除，AOF会显示的追加一条del命令，来显示的记录该key已经被删除
  * **fsync策略**
    * always，表示只要缓冲区中数据发生更改，则就将该数据写入到aof文件中
      * 缺点：三个选项中，最慢的一个
      * 优点：最安全的一个，即使出现故障停机，AOF持久化也只会丢失一个事件循环中所产生的命令数据
    * everysec，每秒写入把缓冲区中数据写入到aof文件中（**redis默认**）
      * 优点：
        * 足够快
        * 出现故障停机，数据库也只会丢失一秒钟的命令数据
    * no，不做任何策略配置，将策略的配置讲给操作系统，一般操作系统是等待缓冲区被占完之后，将数据写入aof文件中。
      * 优点：
        * AOF文件的写入速度是最快的
      * 缺点：
        * 当出现故障停机时，丢失上次同步AOF文件之后的所有写命令数据（缓存中数据）
...
* 存储方式
  * memcache把数据全部存在内存中，断电后会挂掉，数据不能超过内存大小
...
* Hash
  * redis Hash对应value内部实际就是一个HashMap，这里会有2中不同的实现，这个hash的成员比较少时Redis为了节省内存会采用类似一维数组的方式来紧凑存储，而不会采用真正的HashMap结构，对应value redisObject的encoding为zipmap，当成员数量增大时会自动转成真正的HashMap，此时encoding为ht
...
* List
  ...
  * 应用场景
    * 各种列表
    * 消息队列
    * 利用Lrange可以很方便的实现list内容分页功能
    * 取最新N个数据的操作

* Set（无序集合）
  * set内部实现是一个value永远为null的hashMap，实际就是通过计算hash的方式来快速排重的，这也是set能提供判断一个成员是否在集合内的原因
  * 应用场景
    * 某些需要去重的列表
    * 存储一些集合性的数据

* Zset（Sorted Set 有序集合）
  * sorted set的内部使用HashMap和跳跃表（skipList）来保证数据的存储和有序，HashMap里放的是成员到score的映射，而跳跃表里存放的是所有的成员，排序依据是HashMap里存的score，使用跳跃表的结构可以获得比较高的查询效率，并且在实现上比较简单。
  * 应用场景
    * 存放一个有序的并且不重复的集合列表
    * 可以做待权重的队列
    * 排行榜相关

* <https://www.cnblogs.com/starluke/p/11743743.html>
